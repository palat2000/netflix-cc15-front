...
export default function MovieLikeThisBox({ movie }) {
    ...
    return (
        <div className="w-[32%] h-full rounded-md overflow-hidden cursor-pointer">
            <div className="w-full h-full min-h-[155px] bg-neutral-700 relative" onMouseEnter={handleOnMouseEnter} onMouseLeave={handleOnMouseOut} onClick={navigateToWatch}>
            ...
            <div className="flex flex-col w-full h-full bg-neutral-800 min-h-[120px] break-all p-[3%] gap-3">
                <div className="flex items-end justify-between">
                    <h1 className=" text-neutral-400 font-medium" onMouseEnter={handleOnMouseEnter} onMouseLeave={handleOnMouseOut} onClick={navigateToWatch}>{movie?.release_year}</h1>
                    <MyListButton movieId={movie?.id} movieIsInMyListData={movie?.myList} />
                </div>
                <div className="text-neutral-400 text-xs" onMouseEnter={handleOnMouseEnter} onMouseLeave={handleOnMouseOut} onClick={navigateToWatch}>{movie?.detail}</div>
            </div>
        </div>
    )
}
